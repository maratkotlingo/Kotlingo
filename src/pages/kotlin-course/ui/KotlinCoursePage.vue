...
import {
  ...
  type CourseLesson,
  type CourseModule,
} from '@/entities/course/model/course'
  ...
  <div :class="layout.page">
    <CourseTopBar>
      <div class="flex min-w-0 items-center gap-3">
        <img class="h-10 w-10 max-sm:h-8 max-sm:w-8" src="/favicon.svg" alt="" />
        <div class="min-w-0">
          <p class="m-0 text-[11px] font-black uppercase tracking-wide text-muted max-sm:hidden">Kotlin learning track</p>
          <h1 class="m-0 truncate text-2xl font-black leading-none max-sm:text-xl">Kotlingo</h1>
        </div>
      </div>
      ...
      <div class="flex items-center gap-2">
        ...
        <RouterLink to="/compose" :class="[buttons.secondary, 'hidden sm:inline-flex']">
          <span>Compose</span>
        </RouterLink>
        <Button :class="[buttons.icon, 'lg:hidden']" title="Открыть темы" aria-label="Открыть темы" @click="mobileNavOpen = true">
          <Menu :size="20" />
        </Button>
        <Button :class="[buttons.primary, 'max-sm:hidden']" title="Продолжить курс" @click="selectLesson(nextIncompleteLesson)">
          <Play :size="18" />
    ...
    <div :class="layout.contentGrid">
        ...
        <div :class="[layout.panel, 'flex max-h-[inherit] flex-col overflow-hidden']">
          ...
          <div class="grid gap-3 border-b border-line px-5 py-4">
            ...
            <div class="grid grid-cols-[minmax(0,1fr)_42px] gap-2">
              ...
              />
              <Button
                :class="favoritesOnly ? buttons.favoriteActive : buttons.favorite"
          ...
          <nav class="grid gap-5 overflow-auto px-3 pb-5">
            <div v-if="visibleModules.length === 0" class="grid justify-items-center gap-3 px-3 py-8 text-center text-muted">
              <Search :size="22" />
            ...
            <section v-for="(module, moduleIndex) in visibleModules" :key="module.id" class="grid gap-2">
              <button
                ...
                type="button"
                @click="selectCategory(module.title)"
              >
                ...
                <small class="text-xs font-black text-muted">{{ moduleCompletion(module).completed }}/{{ module.lessons.length }}</small>
              </button>
      ...
      <main class="grid min-w-0 gap-4">
        <section class="grid grid-cols-4 gap-3 max-md:grid-cols-2 max-[460px]:grid-cols-1" aria-label="Сводка курса">
          <MetricCard
            v-for="item in [
              { icon: GraduationCap, label: 'Темы', value: courseStats.moduleCount },
              { icon: BookOpen, label: 'Уроки', value: courseStats.lessonCount },
              { icon: Code2, label: 'Практика', value: currentLesson.codeBlocks },
              { icon: Compass, label: 'Прогресс', value: `${completionPercent}%` },
            ]"
            :key="item.label"
            :icon="item.icon"
            :label="item.label"
        ...
        <section class="flex gap-2 overflow-x-auto pb-1" aria-label="Быстрый выбор темы">
          <button
            v-for="(module, moduleIndex) in courseModules"
            :key="module.id"
            :class="[
              'inline-flex min-h-10 max-w-72 shrink-0 items-center gap-2 rounded-control border bg-panel px-3 text-sm font-black transition',
              moduleAccent(moduleIndex).chip,
              categoryFilter === module.title ? moduleAccent(moduleIndex).active : '',
            ]"
            type="button"
            @click="selectCategory(module.title)"
          >
            <span class="truncate">{{ module.title }}</span>
            <small class="text-xs">{{ module.lessons.length }}</small>
          </button>
        </section>

        <article id="lesson-top" :class="[layout.panel, 'min-w-0 overflow-hidden']">
          <div class="flex items-center justify-between gap-4 border-b border-line px-5 py-4 max-sm:flex-col max-sm:items-start">
            <div class="flex min-w-0 items-center gap-2 text-sm font-black text-muted">
              <LayoutList :size="17" />
              <span class="truncate">{{ currentModule.title }}</span>
              <span>/</span>
              <strong class="text-ink">{{ formatOrder(currentLesson.order) }}</strong>
            </div>

            <div class="flex items-center gap-2 max-sm:w-full">
              <Button
                :class="currentLessonFavorite ? buttons.iconActive : buttons.icon"
                :aria-pressed="currentLessonFavorite"
              ...
              <Button
                :class="[currentLessonCompleted ? buttons.completeDone : buttons.complete, 'max-sm:flex-1']"
                :aria-pressed="currentLessonCompleted"
                title="Отметить урок"
          ...
          <header class="border-b border-line bg-gradient-to-b from-panel-raised to-panel px-6 py-9 sm:px-10 lg:px-14">
            <div class="mb-5 flex flex-wrap items-center gap-2">
              ...
              <span class="inline-flex min-h-7 items-center rounded-full border border-line px-3 text-xs font-black text-muted">
          ...
          <footer class="flex items-center justify-between gap-3 border-t border-line p-5 max-sm:flex-col">
            <Button
              :class="[buttons.secondary, 'max-sm:w-full']"
            ...
            <Button
              :class="[buttons.primary, 'max-sm:w-full']"
              :disabled="!nextLesson"
        ...
        <div :class="[layout.panel, 'grid max-h-[inherit] gap-5 overflow-hidden p-4']">
          <div class="grid gap-3 overflow-hidden">
            ...
            <nav class="grid max-h-[42vh] gap-1 overflow-auto">
              <a
                v-for="heading in currentLesson.headings"
                :key="heading.id"
                :class="[
          ...
          <div class="grid gap-3">
            ...
            <div class="grid gap-2 rounded-card border border-line bg-panel-soft p-3">
              ...
              <ProgressBar :value="((currentLessonIndex + 1) / courseStats.lessonCount) * 100" :show-value="false" :pt="progressPt" />
            </div>
          </div>
